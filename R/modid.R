...
#' However, because `D3mirt` analysis is based on item response theory, and uses the multidimensional two-parameter logistic model or the multidimensional graded response model, it is preferable to use item response theory EFA methods,
...
#' The `modid()` takes in the factor solution from the EFA, assigned to a data frame \emph{x}, and outputs lists (denoted \emph{id1]...idn}) with suggestions of items (\emph{item.1....item.n}) to use for the model identification.
...
#'
#' ## Criteria
#' Model identification items should preferably (a) have an absolute sum score of ≤.10 and (b) have the highest factor loading scores on the factor of interest.
