...
#' @param efa The data frame can be explored with exploratory factor analysis using the integrated [mirt::mirt] (Chalmers, 2012) function. The default is `efa = TRUE`.
...
#' @param itemtype The item model for the exploratory factor analysis. Note, only item type 'graded' or '2PL' are allowed. The default is `itemtype = "graded"`. See [mirt::mirt] (Chalmers, 2012) for more on item models.
#' @param method A string indicating what integration algorithm to use. The deafult is `method = 'QMCEM'`. See [mirt::mirt] (Chalmers, 2012) for on methods.
...
#' This orders the model so that the strongest loading item, from the strongest factor, always aligns perfectly with the x-axis and the other items follow thereon.
#'
...
#' The `mirt()`function is integrated into the `modid()` function so that the user needs only to provide the data frame containing empirical data directly in the first argument of the function.
...
#'
#' If raw data is used, the function first performs an EFA, with three factors as default, and then finish with the model identification.
...
#' This includes data frames that hold all the model identification items (`Item.1...Item.n`) selected by `modid()` together with the items absolute sum score (`ABS`), one frame for sum of squares for factors sorted in descending order, and one frame for item factor loadings.
#' The order of the factors follows the model identification items so that item 1 comes from the strongest factor (sorted highest up), item 2 from the second strongest (sorted second), and so on.
#'
...
#' The top items are the items that best meet the necessary statistical requirements for model identification.
