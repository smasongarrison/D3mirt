...
summary.dmirt <- function(x){
  ...
  if (!is.null(x$c.dir.cos)){
    sum <- list(dmirt.est = round(tab1,4), dmirt.angles = round(tab4, 4), construct.angles = round(tab5, 4))
  } else{
    sum <- list(dmirt.est = round(tab1,4), dmirt.angles = round(tab4, 4))
  }
  if (!is.null(x$c.dir.cos)){
