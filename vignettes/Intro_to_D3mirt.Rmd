...
The `D3mirt` analysis is based on descriptive multidimensional item response theory (DMIRT; Reckase2009, 1985; Reckase & McKinley, 1991) and can be used to analysis dichotomous and polytomous items in a multidimensional ability space.
...
For `D3mirt`, this implies that a three-dimensional GRM must be fitted so that discrimination and difficulty parameters for a three-dimensional latent trait model can be extracted.
...
The item orientation is set by taking the direction cosines, using linear algebra terms, of $a_{il}$, i.e., the slope values of item $i$ on coordinate axis $l$ (Reckase, 2009).
...
However, in DMIRT analysis, the MDIFF becomes a multidimensional location parameter that indicate the distance from the origin to the point of maximum slope following the direction vector given by the $a_{il}$ equation.
...
But also because only two items are necessary for a adequate model identification (described below).
...
The first item should not load on the second and third axes ($y$ and $z$), while the second item should not to load on the third axis ($z$).
If this can be empirically achieved, it will be possible to create a orthogonal structure, that is is necessary for the analysis.
...
Because of this, the `modid` function was developed to maximize the interpretive meaning analytically by first order factors by sum of squares and from this select the strongest loading items that meet the statistical assumptions described above.
This orders the model so that the strongest loading items, from the strongest factor, always align with the x-axis and that the other items follows thereon.
...
Accordingly, the top items in each list are the items that best meet the necessary statistical requirements for the model identification.
...
As can be seen, the first item, that will be used set the x-axis, is found in the first list, `list 1` and `item.1`.
...
Adjusting this limit to high means that the necessary statistical requirements are compromised.
...
Another option if the analysis runs in to problems is to override the factor order with the `fac.order` argument.
...
Since the squared factor loadings for factor 2 and 3 are quite similar, it can be useful to try this suggestion as well and then compare the results.
...
Items are selected by scaling $f_1$, and then using a standard deviation of $0.5$ (can be adjusted with the `lower` argument.) as the lower bound criteria for inclusion in the item pool.
That is, starting form the top, rows with raw factor scores and absolute sum scores are extracted until the lower bound is reached.
...
The `D3mirt()` function takes in an data frame with model parameters from a three-dimensional GRM model, specified with orthogonal factors and set so that all items load on all factors (see examples below).
The function then returns an $S3$ object of class `D3mirt` with lists of $a$ and $d$ parameters from the GRM, as well as $MDISC$, $MDIFF$, direction cosines and degrees for vector angles, construct lists, and vector coordinates assesed by `D3mirt()`.
...
As cabn be seen, the summary function gives starts with a short statements on the number of items and difficulty levels.
Next list of estimates are shown.
...
The construct vector arrows can contribute to the analysis by (a) visualizing average direction for a subset set of items, and (b) to show how combinations of items interrelate on group level, i.e., as a type of dimensionality analysis.
Examples of the utility of using constructs will be given in short analysis example presented below.
...
In DMIRT (Reckase, 2009, 1985, Reckase & McKinley, 1991), the angle of the vector arrows, seen from the model axes, indicates the direction of maximal slope of discrimination for the particular item response function.
...
Graphing in default mode by calling `plotd3mirt` will return a RGL device that will appear in an external window as an three-dimensional object that can be rotated.
...
This was only done for optimizing the size of RGL device (default `view = c(15, 20, 7)`) when creating this R Markdown.
...
The graphical device indicate the presence of foremost two latent dimensions, one aligning with the x-axis and one approaching the y-axis.
The two dimensions, however, have some within-multidimensional tendencies since they are not completely orthogonal.
This tendency, however, is much more prominent when looking at the items along the z-axis.
...
However, grouping of items can be exploratory, as in this case, or based on theory.
Note, the item W7Q16 was not included in of the construct because of signs of model violations and within-multidimensionality.
...
The output visualizes the average direction of the item vectors.
Moreover, the angles of the construct vector informs us on how these groups of items are associated.
...
As was mentioned above, the W7Q16 had the most problems of all items in the data.
More specifically, the short vector arrows indicate model violations.
Moreover, the location of the item in the model also indicate that the item is within-multidimensional and that it does not seem to belong to any construct explicitly.

The `plotD3mirt` allows us to plot the item alone using argument `items` and entering the number indicating were the item appears in the data set (see `?anes08_09offwaves`).
...
The item is located at $x° = 35.89, \, y° = 57.47, \, z° = 76.49$ which indicates that it is within-multidimensional with respect to the x and y-axis and can also be seen to be oriented between Compassion and Conformity.
The low discrimination ($MDISC = .7699, \, MDIFF_{range} = [3.72, -1.81]$) is the lowest of all items in the solution, which, compared to the other items in the model, implies that the item does not seem to fit the three-dimensional GRM used in this particular analysis.
We should also note that item W7Q15 ($x° = 60.47, \, y° = 31.52, \, z° = 100.02, \, MDISC = .9232,\, MDIFF_{range} = [4.32, -.97]$) suffer from a low discrimination score but is not as within-multidimensional.
...
Items are selected, just as in the sinlge case, based on order in the data frame (see `?anes08_09offwaves`)
...
This can be described by dividing the model into octants, $O_{1-8}$, and then see what octants the respondents in question tend to occupy.
For example, if more respondents, conditioned on some external variable, are located within one particular octant, $O_n$, this indicates that the respondents tend to have the latent profile that $O_n$ describes in the model, e.g., high on trait $x$ (above center on the x-axis), low on trait $y$ (below center on the y-axis) and high on trait $z$ (above center on the z-axis).
And so on.
...
This means that raw data can be used but the number of colors in the color vector (`sphere.col`) may need to be adapted.
...
In the graphical output it can be observed a singular profile on gender in which more women tend to have higher levels of trait on compassion.
There seems not be any observable gender difference related to Conformity or Fairness.
...
The result graphical output indicate an age effect in which older individuals have higher levels of trait on Conformity.
...
There does not seem to be any difference on Compassion, however. 
Thus, the analysis indicate a possibility that older individuals may have a moral profile in the model that consist of high Conformity and high Fairness. 
