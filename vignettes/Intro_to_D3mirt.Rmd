...
In the `D3mirt`package the $DDISC$ is computed for all items following the angle orientation given by the *construct vectors*, i.e., user adjusted optional vectors whose angle direction is calculated as the average direction of a set of items in the model.
...
The sections below are sorted under the following headlines.
...
Thus, the EFA model itself is discarded after this step in the procedure.
...
The `modid()` takes in the factor solution from the EFA, assigned to a data frame $x$, and outputs an $S3$ object of class `modid` containing lists with data frames of estimates.
The most important is the item lists, (denoted `$items`), containing data frames that present suggestions on what items (`item.1`...`item.n`) to use for the model identification.
...
As can be seen, the first item, that will be used to identify the $x$-axis, is found in the first list, `items[[1]]` and `item.1`.
...
Next, items are selected by first scaling $f_1$ loadings and then extracting the items with the highest loadings on $f_1$, up to a standard deviation of $0.5$ as the lower bound criteria counting from the top.
...
The `D3mirt()` function takes in a data frame with model parameters from a three-dimensional compensatory model and returns an $S3$ object of class `D3mirt` with lists of $a$ and $d$, $MDISC$, and $MDIFF$ parameters, direction cosines, and degrees for vector angles. If constructs are used, the function also returns construct vector angles and $DDISC$  parameters, one index per construct, for all items. 

### Fitting the Compensatory Multidimensional Model

The three-dimensional compensatory model is specified so that all items load on all three factors in the model, and that the factors are constrained to be orthogonal (see examples below).
...
Constructs are included in the analysis by creating one or more nested lists that indicate what items belong to what construct.
From this, the `D3mirt()` function finds the average direction of the subset of items contained in each construct list, by adding and normalizing the direction cosines for the items.
...
Compared to the previous function call to `D3mirt()`, we now see two extra lists: `$construct.angles` with direction cosines and angles for the constructs, and `$ddisc` containing the $DDISC$ parameters for the items on the three construct imputed with *c* in the example above.
...
The output consists of a three-dimensional interactive RGL device displaying vector arrows with the latent dimensions running along the orthogonal, centered at zero.
...
> As can be seen, the item vector clustering patterns in Figure X indicate the presence of foremost two latent dimensions, one aligned with the $x$-axis and one approaching the $y$-axis. The two dimensions, however, have some within-multidimensional tendencies since they are not completely orthogonal. This tendency, however, is much more prominent for the items W7Q11, W7Q12, W7Q13, and W7Q14, being located at around the $45°$ angle or higher between the $x$ and $z$-axis.
...
> The output in Figure $X$ shows that Compassion ($x° = 3.91, y° = 86.49, z° = 88.29$) and Conformity ($x° = 61.84, y° = 28.19, z° = 91.19$) have some within-multidimensional tendencies.
> However, they are both more or less orthogonal to the $z$-axis.
> Next, we find Fairness ($x° = 50.13, y° = 78.31, z° = 42.25$) with clear within-multidimensional tendencies with respect to the $x$-axis.
...
> Figure $X$ shows that item W7Q16 is located at $x° = 35.891, y° = 57.473, z° = 76.493$ which indicates that the item is within-multidimensional with respect to the $x$ and $y$-axis but approaching orthogonal tendencies with respect to the $z$-axis.
...
> However, the latter item does seem to belong to the Conformity construct when comparing angle orientation ($x° = 60.466, y° = 31.516, z° = 100.017$) and discrimination ($MDISC = .923, DDISC_1 = .332, DDISC_2 = .912, DDISC_3 = .497$).
