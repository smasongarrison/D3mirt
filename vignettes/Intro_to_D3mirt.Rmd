...
The `D3mirt` analysis is based on descriptive multidimensional item response theory (DMIRT; Reckase2009, 1985; Reckase & McKinley, 1991) and can be used to analyze dichotomous and polytomous items (see Muraki & Carlson, 1995) in a multidimensional ability space.
The method is visual and illustrates item characteristics, represented by vector arrows, located in a three-dimensional interactive space.
...
The methodology is a type of data reduction technique that uses a compensatory model (Reckase, 2009), i.e., a model that assumes the existence of a multidimensional trait space forming an interconnected latent ability field.
...
Moreover, the DMIRT analysis is said to be *descriptive* because the results describe the extent to which items in a test are unidimensional, i.e., that the items describe one dimension only, or are within-multidimensional, i.e., that the items describe more than one dimension.

The DMIRT approach uses two types of item models, dependent on item type.
If dichotomous items are used, the analysis is based on the two-parameter logistic model (2PL).
If polytomous items are used, the analysis is extended to the two-parameter graded response model (GRM; Samejima, 1969).
In both cases, the estimation process consists of first fitting a compensatory multidimensional two-parameter model so that discrimination and difficulty parameters can be extracted.
...
The $MDISC$ for item $i$ represents the highest level of discrimination the item $i$ can achieve located in a multidimensional latent trait space, with $m$ number of dimensions and $a_{ik}$ item slope parameters (Reckase2009, 1985; Reckase & McKinley, 1991).

::: {align="center"} $MDISC = A_i = \sqrt {\sum_{k = 1}^{m} a^{2}_{ik} }$ :::

Similarly to unidimensional modeling, the $MDISC$ parameter, also denoted as $A_i$, sets the steepness of the slope of the item response function at the point of inflection.
The slope is, similarly to the unidimensional case, assessed as using the constant $\frac{A_i}{4}$ (omitted in the equation above).

The item orientation is set by taking the direction cosines, using linear algebra terms, of $a_{il}$, i.e., the slope values of item $i$ on the coordinate axis $l$ (Reckase2009, 1985; Reckase & McKinley, 1991).
...
The multidimensional version of the difficulty parameter, here denoted $D_i$, for item $i$, is defined as the negative intercept $d_i$ divided by the $MDISC$ (Reckase2009, 1985; Reckase & McKinley, 1991).
...
However, in DMIRT analysis, the $MDIFF$ becomes a multidimensional location parameter that indicates the distance from the origin to the point of inflection following the direction vector given by the $a_{il}$ equation.

For Likert items that hold multiple item response functions, the $MDIFF$ will be turned into an index that indicates the location of the point of inflection for all response functions derived from an item.
This, in turn, implies that the $MDIFF$ will show the *difficulty range* for an item in the model.

The $MDISC$ is visualized in DMIRT analysis by using it to scale the length of the vector arrows.
In brief, the bottom location coordinates of the vector arrows, given by the $MDIFF$ and $a_{il}$, are multiplied with the $MDISC$ so that items with higher $MDISC$ have longer vector arrows (Reckase, 2009).
...
The $DDISC$ describes the level of discrimination for one or more items in the model at the angle set by the direction cosines of choice.
Note, it is always true that $DDISC ≤ MDISC$.
...
In `D3mirt`, constructs are implemented as user adjusted optional vectors whose angle direction is calculated as the average direction of a set of items in the model.
Item sets are created by the user and can be motivated based on exploratory reasons (e.g., grouping items based on model observations) or theoretical reasons (e.g., personality theory).
Accordingly, the computation of the $DDISC$ index, following the construct vectors, allows the researcher to compare item discrimination locally under the assumption that these constructs exist.

## Limitations

The DMIRT method is currently limited to using compensatory models and two-parameter item models.
...
This since the third axis, the $z$-axis, is freely estimated and because only two items in the set must meet the necessary statistical assumptions for identifying the compensatory model (see the section on model identification below for more on the statistical requirements).
...
The `modid()` function was, therefore, developed to maximize the interpretive meaning by using an algorithmic approach.
...
However, because `D3mirt` analysis is based on the 2PL and the GRM, it is recommended to use multidimensional item response theory EFA methods, such as the EFA option in `mirt::mirt` (Chalmers, 2012) with `ìtemtype = 'graded'` or `'2PL'`, so that the EFA is performed with the proper item model.
...
Note, because the EFA in the `mirt()` function takes a long time to perform, the item loadings from the EFA for this example are stored in the package file "efa.Rdata" that will loaded in the next code chunk below.
...
```{r}
# Load the EFA data for this example directly from package file
load("efa.Rdata")
...
The list is rearranged in decreasing order based on factor loading strength on $f_1$.

Next, items are selected by first scaling $f_1$ loadings and then extracting the items with the highest loadings on $f_1$, up to a standard deviation of $0.5$ (default) as the lower bound criteria counting from the top.
...
However the absolute sum score is always assessed based on the number of factors less than the total number of factors, following the order of iteration, That is, iteration $1$ use factor loadings from all factors $F-f_1$, iteration $2$ $F-(f_{1-2})$, iteration $3$ $F-(f_{1-3})$, and so on, when calculating the absolute sum scores.
...
Note, that two items can be found to identify the model does not imply successful outcomes when using this methodology (i.e., that the model is *good*).
...
The `D3mirt()` function takes in a data frame with model parameters from a three-dimensional compensatory model and returns an $S3$ object of class `D3mirt` with lists of $a$ and $d$, $MDISC$, and $MDIFF$ parameters, and direction cosines..
The direction cosines are also reported in spherical coordinates $\theta$ and $\phi$ ($\rho = 1$).
...
Note, the positive $\theta$ angle is oriented following the positive poles of the $x$ and $z$ axis so that the angle increases clockwise in the graphical output.
In opposite direction, the negative $\theta$ angle is oriented following the negative poles of the $x$ and $z$ axis so that the angle increases counterclockwise in the graphical output.
...
The three-dimensional compensatory model is specified so that all items load on all three factors in the model, and that the factors are constrained to be orthogonal (see below).
The fitting of the model is preferably done with the `mirt::mirt` (Chalmers, 2012) function.
Please note very carefully regarding the model specification in the example below.
Note, because the fitting of the compensatory model in the `mirt()` function takes a long time to fit, the item parameters are available in the package file "d.Rdata" that will be loaded in the next code chunk below.
...
The output, however, is lengthy so the use of the summary function when inspecting the results is recommended.

```{r}
# Load data frame d for for this example directly from package file
load("d.Rdata")
...
From this, the `D3mirt()` function finds the average direction of the subset of items contained in each construct list, by normalizing the direction cosines for the items.
Note, the length of the vector arrows is arbitrary and can be user adjusted.
...
In the following two sections, some theory will be given to help interpret the graphical output.
...
For instance, in Cartesian coordinates, an item located at $0°$ seen from the $x$-axis, and $90°$ as seen from the $y$ and $z$-axis, only describes trait $x$.
...
Graphing in default mode by calling `plotd3mirt()` will return an RGL device that will appear in an external window as a three-dimensional interactive object that can be rotated manually.
In this illustration, however, all RGL devices are plotted as inline interactive objects.
The `view` argument is also used in all function calls to `plotD3mirt()` below.
This was only done for optimizing the size of the RGL device (default `view = c(15, 20, 7)`) when creating this R Markdown vignette.
...
> As can be seen, the item vector clustering patterns in Figure $X$ indicate the presence of foremost two latent dimensions, one aligned with the $x$-axis and one approaching the $y$-axis.
> The two dimensions, however, seem to have some within-multidimensional tendencies since they are not completely orthogonal.
> This tendency, however, is much more prominent for the items W7Q11, W7Q12, W7Q13, and W7Q14, being located at around $\theta = 45°$ or higher between the $x$ and $z$-axis.
...
In this example, items have been grouped exploratory, i.e., based on their observed location in the model.
...
> The output in Figure $X$ shows that Compassion ($\theta = 1.710°, \phi = 86.486°$) and Conformity ($\theta = -2.519°,\phi = 28.192°$) have some within-multidimensional tendencies.
> However, they are both more or less orthogonal to the $z$-axis ($\theta = 1.710°$ and $\theta = -2.519°$ respectively).
> Next, we find Fairness ($\theta = 49.106°, \phi = 78.308°$) with clear within-multidimensional tendencies with respect to the $x$-axis.
...
> Figure $X$ shows that item W7Q16 is located at $\theta = 16.083°, \phi = 57.473°$, which indicates that the item is within-multidimensional with respect to the $x$ and $y$-axis but approaching orthogonal tendencies with respect to the $z$-axis.
...
> The global discrimination score ($MDISC = .770, MDIFF_{range} = [-4.838, 2.349]$) is also the lowest of all discrimination scores in the model.
> This, combined, implies that the item in question does not seem to fit the three-dimensional compensatory model used in this analysis.
> We should also note that item W7Q15 ($MDISC = .923, MDIFF_{range} = [-4.680, 1.051]$) has the second lowest discrimination score.
> However, the latter item does seem to belong to the Conformity construct when comparing angle orientation ($\theta = -19.436°, \phi = 31.516°$) and discrimination ($MDISC = .923, DDISC_1 = .332, DDISC_2 = .912, DDISC_3 = .497$).

### Other Graphical Options

#### `diff.level`

The user has the option of plotting on one level of difficulty at a time with the `diff.level` argument on the entire scale or on one item at a time.
Note, *difficulty* is the number of item response functions, i.e., the total number of response options minus one.
...
Items are selected, just as in the single case, based on the item order in the original data frame (see `?anes08_09offwaves`)
...
The `plotD3mirt()` function can also display respondents in the three-dimensional model represented as spheres located with respondent factors scores used as coordinates.
This allows for a profile analysis in which respondent rows can be selected conditioned on single or multiple criteria and then plotted.
...
To do this, the user must first extract respondent factor scores with `mirt::fscores` (Chalmers, 2012) and then select a subset of respondent rows based on one or more criteria.
...
```{r}
# Load factor scores for respondents for this example directly from package file
load("fscores.Rdata")
...
> When rotating the model, there seems not to be any observable gender difference related to Conformity or Fairness.

The use of `rep()` function makes it possible to create groups based on factor levels.
This can be useful when a criterion variable has a wide data range, such as an age variable for example.
...
In the example below, the `nlevels(as.factor(x))` is used to count the number of factor levels in the raw data.
The output is then used to create the appropriate color vector for `sphere.col`.
...
> To sum, we can say, therefore, that the analysis indicates a possibility that older individuals may have a moral profile consisting of high Conformity and possibly also high Fairness.

It is also possible to plot a confidence interval in the shape of an ellipse surrounding the individual scores.
In the example below, the younger individuals ($≤30$) are selected and plotted together with a $95\%$ *CI*.
...
The color of the sphere was also changed from default `grey80` to `orange` in the example below.
