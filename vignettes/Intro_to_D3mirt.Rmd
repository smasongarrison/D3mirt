...
In `D3mirt`, constructs are implemented as optional vectors whose orientation is calculated as the average direction, i.e., the average multidimensional discrimination of a subset of items in the model.
...
> As can be seen in the figure, the pattern in the data indicates the presence of foremost two main latent dimensions indicated by the items, one aligned with the $x$-axis and one approaching the $y$-axis.
> We might also suspect the presence of a third dimension located close to the $xy$-plane, between the $x$ and $z$ axes.
...
> In the figure, it can be observed a simple profile on gender in which more women tend to have higher levels of trait on Compassion.
> When rotating the model $90^{\circ}$ clockwise, there seems not to be any easily observable gender difference related to Conformity or Fairness.
