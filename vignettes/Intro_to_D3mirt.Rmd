...
When creating this document, extracted model parameters from the S4 object `mod1`, estimated in the previous step, were assigned to a data frame with the same name and is used in the call to `D3mirt()`. Note, however, it is also possible to use `mod1` as an S4 object directly in the call to `D3mirt()`.
