...
When calling `D3mirt()` it is possible to use the S4 object from `mirt()` directly in the call to `D3mirt()` or a data frame containing model parameters from the compensatory model. The `summary()` function, included in the package, is used to inspect the DMIRT estimates. The constructs included below were grouped based on exploratory reasons, i.e., because these items cluster in the model (observable in the graphical output below).
